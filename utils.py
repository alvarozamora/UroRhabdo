...
def Loss(pred, strain, prob, ptrain, stest, ptest, model, xtest, epoch, best):
...
def AUCplot(spec_model, over_model, spec_xtest, over_xtest, yspec, yover, spec_rocs, over_rocs, k, K):
	...
	if (k+1) == K:
		...
		both_ax.plot(mean_fpr, mean_spec_roc, color ='C0', label=f'Mean DSS AUC = {mean_auc:.2f} $\pm$ {std_auc:.2f})')
		both_ax.fill_between(mean_fpr, spec_bot, spec_top, color='C0', alpha=.1)#, label=f'$\pm$ 1 DSS std')
		both_ax.plot(mean_fpr, mean_over_roc, color ='C1', label=f'Mean Overall AUC = {mean_auc:.2f} $\pm$ {std_auc:.2f})')
		...
		both_ax.title.set_text("DSS and Overal Survival Models")
